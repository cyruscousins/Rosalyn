...
class Program a b where
  ...
  programDirecory :: String
  programDirectory = binaryName ++ "/" ++ subName ++ "/"
  executionSubirectory :: a -> String
  fullDirectory :: a -> String
  programDirectory
  produceDirectory input = memozationRoot ++ 
  arguments :: a -> [String]
  

--TODO specialize for Hashable a
...
ghcInput = fmap (const ()) (return "Remy please implement this.")
